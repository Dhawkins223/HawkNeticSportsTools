...
class SportsResearchTests(PostgresTestCase):
    ...
    def test_predictions_settle_and_report_keeps_win_rate_unavailable_until_settled(self) -> None:
        ...
        self.assertEqual(candidates[0]["model_state"], "baseline_only")
        self.assertEqual(candidates[0]["probability_source"], "bookmaker_implied")
        self.assertEqual(candidates[0]["decision_status"], "track_only")
        self.assertEqual(candidates[0]["features"]["decision_reason"], "market_price_observation_not_validated_model")
