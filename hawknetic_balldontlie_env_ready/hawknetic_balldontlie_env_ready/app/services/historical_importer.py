...
class HistoricalImporter:
    def __init__(self, raw_job_id: int | None = None) -> None:
    ...
    def _log_row_error(self, season: int, table_name: str, row: dict[str, str], exc: Exception) -> None:
        if not self.raw_job_id:
            return
        identifier = row.get("game_key") or row.get("player_key") or row.get("team_key") or row.get("external_id") or row.get("full_name") or str(row)[:120]
        RawImportTracker.log_error(self.raw_job_id, "basketball_reference", str(exc), season=season, table_name=table_name, row_identifier=identifier)
    ...
    def _import_teams(self, conn: Any, rows: list[dict[str, str]]) -> int:
        ...
        for row in rows:
            try:
                ...
                self._log_row_error(0, "historical_teams", row, exc)
        return inserted
    ...
    def _import_players(self, conn: Any, rows: list[dict[str, str]]) -> int:
        ...
        for row in rows:
            try:
                ...
                self._log_row_error(0, "historical_players", row, exc)
        return inserted
    ...
    def _import_schedule(self, conn: Any, season: int, rows: list[dict[str, str]]) -> int:
        ...
        for row in rows:
            try:
                ...
                self._log_row_error(season, "historical_games", row, exc)
        return inserted
    ...
    def _import_player_season(self, conn: Any, season: int, rows: list[dict[str, str]], entity_type: str) -> int:
        ...
        for row in rows:
            try:
                ...
                self._log_row_error(season, "historical_season_stats", row, exc)
        return inserted
    ...
    def _import_player_advanced(self, conn: Any, season: int, rows: list[dict[str, str]]) -> int:
        ...
        for row in rows:
            try:
                ...
                self._log_row_error(season, "historical_player_ratings", row, exc)
        return inserted
    ...
    def _import_team_games(self, conn: Any, season: int, rows: list[dict[str, str]]) -> int:
        ...
        for row in rows:
            try:
                ...
                self._log_row_error(season, "historical_team_game_stats", row, exc)
        return inserted
    ...
    def _import_player_games(self, conn: Any, season: int, rows: list[dict[str, str]]) -> int:
        ...
        for row in rows:
            try:
                ...
                self._log_row_error(season, "historical_player_game_stats", row, exc)
        return inserted
