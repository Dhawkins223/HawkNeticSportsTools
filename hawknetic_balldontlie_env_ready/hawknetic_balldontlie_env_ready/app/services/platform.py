...
class PlatformService:
    ...
    def dashboard_snapshot() -> dict:
        with get_connection() as conn:
            return {
                "teams": conn.execute("SELECT COUNT(*) c FROM canonical_teams").fetchone()["c"],
                "players": conn.execute("SELECT COUNT(*) c FROM canonical_players").fetchone()["c"],
                "games": conn.execute("SELECT COUNT(*) c FROM canonical_games").fetchone()["c"],
    ...
    def list_teams(limit: int = 30) -> list[sqlite3.Row]:
        with get_connection() as conn:
            return conn.execute("SELECT * FROM canonical_teams ORDER BY full_name ASC LIMIT ?", (limit,)).fetchall()

    @staticmethod
    def get_team(team_id: int):
        with get_connection() as conn:
            team = conn.execute("SELECT * FROM canonical_teams WHERE id=?", (team_id,)).fetchone()
            if not team:
                return None
            recent_games = conn.execute(
                """
