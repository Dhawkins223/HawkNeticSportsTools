...
### Basketball Reference rate limiting guidance

Basketball Reference can return `429 Too Many Requests` if multiple pages are fetched too quickly (common on Railway).
The scraper now performs polite throttling and 429 retries:

- `HAWKNETIC_BREF_REQUEST_DELAY_SECONDS` (default `12`)
- `HAWKNETIC_BREF_MAX_RETRIES` (default `3`)
- `HAWKNETIC_BREF_429_COOLDOWN_SECONDS` (default `1800`)

Recommended Railway values:

```bash
HAWKNETIC_BREF_REQUEST_DELAY_SECONDS=12
HAWKNETIC_BREF_MAX_RETRIES=3
HAWKNETIC_BREF_429_COOLDOWN_SECONDS=1800
```

If the scraper is rate-limited, the API returns:
`Basketball Reference rate-limited the scraper. Wait before retrying.`
and import is skipped.

For first validation, run one-season test only (2024) from Admin / Testing Controls, then fetch `2024` scrape errors in the same panel.
